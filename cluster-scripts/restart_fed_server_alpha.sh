...
for i in "${!PeerAddress[@]}"; do
  ...
  ssh ${HostUser}@${addrIN[0]} "(cd ~/EASC/federated-learning/; python3 -u fed_server_alpha.py --model=cnn --dataset=realworld --hyperpara=0.75) > ~/EASC/server_${addrIN[0]}.log 2>&1 &"
done
