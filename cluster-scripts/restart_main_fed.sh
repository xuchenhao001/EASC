...
function restartService() {
	...
	nohup python3 -u main_fed.py > server_10.137.3.70.log 2>&1 &
	cd -
	for i in ${!AllNodesAddrs[@]}; do
		index=$(printf "%02d" $((i+2)))
		ssh ubuntu@${AllNodesAddrs[$i]} " kill -9 \$(ps -ef|grep '[m]ain_fed.py'|awk '{print \$2}')"
		ssh ubuntu@${AllNodesAddrs[$i]} "(cd ~/EASC/federated-learning-master/; python3 -u main_fed.py) > server_${AllNodesAddrs[$i]}.log 2>&1 &"
	done
}
