...
function main() {
	for i in "${!TestSchema[@]}"; do
		schema=(${TestSchema[i]//-/ })
		echo "ALL_NODE_TEST UNDER: ${schema[0]} - ${schema[1]}"

		# 1. fed_server
		echo "## fed_server start ##"
		# clean
		clean
		# run test
		...
		echo "## fed_server done ##"
		
		# 2. fed_server_alpha
		echo "## fed_server_alpha start ##"
		# clean
		clean
		# run test
		...
		echo "# fed_server_alpha_025 done #"
		...
		echo "# fed_server_alpha_050 done #"
		...
		echo "# fed_server_alpha_075 done #"
		echo "## fed_server_alpha done ##"

		# 3. main_fed_localA
		echo "## main_fed_localA start ##"
		# clean
		clean
		# run test
		./restart_fed_server.sh ${schema[0]} ${schema[1]}
