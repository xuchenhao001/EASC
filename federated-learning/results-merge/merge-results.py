...
def merge_files(user_number, merge_files_path):
    ...
    merged_file_name = os.path.join(merge_files_path, "merged.txt")
...
def main():
    ...
    for dirpath, dirs, files in os.walk("./output"):
        ...
        if len(files) > 0 and files[0].endswith('.txt'):
            resut_files = [file for file in files if not file.endswith('merged.txt')]
            user_number = len(resut_files)
            merge_files(user_number, dirpath)
