...
function releaseCerts() {
  ...
  for i in ${!AllNodesAddrs[@]}; do
    ...
    ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-20-peers/network-node${index} && ./network.sh up && cd ~/EASC/federated-learning-master/ && nohup python3 -u fed_server.py > server_${AllNodesAddrs[$i]}.log 2>&1 &"
  done
...
function networkDown() {
  ...
  for i in ${!AllNodesAddrs[@]}; do
    ...
    ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-20-peers/network-node${index} && ./network.sh down && kill -9 $(ps -ef|grep '[p]ython3 -u fed_server.py' | awk '{print $2}')"
  done
}
