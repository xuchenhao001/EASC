...
function releaseCerts() {
  ...
  for i in ${!AllNodesAddrs[@]}; do
    ...
    # scp peerOrganizations.tar.gz ubuntu@${AllNodesAddrs[$i]}:~/EASC/fabric-samples/network-30-peers/network-node${index}/peerOrganizations.tar.gz
    # ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-30-peers/network-node${index} && tar -zxf peerOrganizations.tar.gz && rm -f peerOrganizations.tar.gz && ./network.sh up"
    ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-30-peers/network-node${index} && ./network.sh up && cd ~/EASC/federated-learning-master/ && nohup python3 -u fed_server.py > server_${AllNodesAddrs[$i]}.log 2>&1 &"
  done
...
function networkDown() {
  ...
  for i in ${!AllNodesAddrs[@]}; do
    index=$(printf "%02d" $((i+2)))
    # ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-30-peers/network-node${index} && ./network.sh down && rm -rf organizations/"
    ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-30-peers/network-node${index} && ./network.sh down && kill -9 $(ps -ef|grep '[p]ython3 -u fed_server.py' | awk '{print $2}')"
  done
}
