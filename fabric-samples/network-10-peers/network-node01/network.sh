...
function networkDown() {
  ...
  for i in ${!AllNodesAddrs[@]}; do
    index=$(printf "%02d" $((i+2)))
    ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-10-peers/network-node${index} && ./network.sh down && rm -rf organizations/ && kill -9 \$(ps -ef|grep '[f]ed_server.py'|awk '{print \$2}')"
  done
}
