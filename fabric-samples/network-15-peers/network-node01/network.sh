...
function releaseCerts() {
  ...
  for i in ${!AllNodesAddrs[@]}; do
    ...
    scp peerOrganizations.tar.gz ubuntu@${AllNodesAddrs[$i]}:~/EASC/fabric-samples/network-15-peers/network-node${index}/peerOrganizations.tar.gz
    ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-15-peers/network-node${index} && tar -zxf peerOrganizations.tar.gz && rm -f peerOrganizations.tar.gz && ./network.sh up"
  done
...
function networkDown() {
  ...
  for i in ${!AllNodesAddrs[@]}; do
    index=$(printf "%02d" $((i+2)))
    ssh ubuntu@${AllNodesAddrs[$i]} "cd ~/EASC/fabric-samples/network-15-peers/network-node${index} && ./network.sh down && rm -rf organizations/"
  done
  # clean wallet
